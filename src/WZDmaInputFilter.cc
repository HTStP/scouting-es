...
WZDmaInputFilter::WZDmaInputFilter(size_t packetBufferSize,
...
inline ssize_t WZDmaInputFilter::read_packet(char **buffer, size_t bufferSize) {
  ...
  while (bytesRead > (ssize_t)bufferSize) {
    ...
    skip++;
    LOG(ERROR) << "#" << nbReads() << ": DMA read returned " << bytesRead
               << " > buffer size " << bufferSize << ". Skipping packet #"
               << skip << '.';
    if (skip >= 100) {
      reset++;
      stats.nbBoardResets++;
      ...
      if (reset > 10) {
        ...
        throw std::runtime_error(
            "FATAL: DMA is still returning large packets.");
      }
...
void WZDmaInputFilter::print(std::ostream &out) const {
  out << ", DMA errors " << stats.nbDmaErrors << ", oversized "
      << stats.nbDmaOversizedPackets << ", resets " << stats.nbBoardResets;
}
